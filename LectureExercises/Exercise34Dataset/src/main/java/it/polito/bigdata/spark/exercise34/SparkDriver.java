...
/*
		...
		Dataset<Reading> selectedReadings =  dsReadings.filter(r -> r.get_c2()==maxValue.doubleValue());
	...
	*/
		...
		SparkSession ss = SparkSession.builder().appName("blabla").getOrCreate();
		
		Dataset<Reading> ds = ss.read().format("csv").option("header", false).option("inferSchema",true).load(inputPath).as(Encoders.bean(Reading.class));
		Dataset<Double> dsMax = ds.agg(max("_c2")).as(Encoders.DOUBLE());
		
		Double maxValue = dsMax.first();
		
		Dataset<Reading> filteredDs = ds.filter(line->{
			if(line.get_c2()==maxValue)
				return true;
			else {
				return false;
			}
		...
		filteredDs.write().format("csv").save(outputPath);
		ss.stop();
	}
